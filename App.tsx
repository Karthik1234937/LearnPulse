...
const App: React.FC = () => {
  ...
  const fetchUserProfile = useCallback(async (authUser: any, isSignup: boolean = false) => {
    if (!isSupabaseConfigured) return;
    try {
      const { data, error } = await supabase
        .from('profiles')
      ...
      if (error) {
        console.warn("Profile fetch failed, trying localStorage:", error);
        // Try localStorage fallback
        const storedProfile = localStorage.getItem(`profile_${authUser.id}`);
        if (storedProfile) {
          const profileData = JSON.parse(storedProfile);
          const userData: User = {
            name: profileData.full_name || authUser.user_metadata?.full_name || 'Learner',
            email: profileData.email || authUser.email || '',
            avatar: `https://api.dicebear.com/7.x/avataaars/svg?seed=${authUser.id}`,
            joinedDate: new Date(authUser.created_at || new Date()).toLocaleDateString(),
            occupation: profileData.occupation || '',
            learningGoal: profileData.learning_goal || '',
            preferredLevel: 'Beginner',
            enrolledCourseIds: profileData.enrolled_ids || []
          };
          setUser(userData);
          setEnrolledIds(profileData.enrolled_ids || []);
          return;
        }
        throw error;
      }
      ...
      if (!data && isSignup) {
         try {
           ...
           console.warn("Profile upsert failed, storing locally:", upsertError);
           ...
           localStorage.setItem(`profile_${authUser.id}`, JSON.stringify(profileData));
         }
      }

      // Try to get enrolled_ids from database, fallback to localStorage
      let dbEnrolledIds: string[] = [];
      if (data?.enrolled_ids) {
        dbEnrolledIds = data.enrolled_ids;
      } else {
        // Fallback to localStorage
        const stored = localStorage.getItem(`enrolled_courses_${authUser.id}`);
        if (stored) {
          try {
            dbEnrolledIds = JSON.parse(stored);
          } catch (e) {
            console.error("Failed to parse stored enrolled courses:", e);
          }
        }
      }
      ...
      const userData: User = {
        ...
        enrolledCourseIds: dbEnrolledIds
      };
      
      setUser(userData);
      setViewedCourses(data?.saved_courses || []); 
      setEnrolledIds(dbEnrolledIds);
  ...
  }, [fetchHistory, name, occupation, learningGoal]);
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (authMode === 'SIGNUP') {
        ...
        // Sign up without email verification for immediate access
        ...
        if (signUpData.user) {
          ...
          try {
            await fetchUserProfile(signUpData.user, true);
          } catch (profileError) {
            console.warn("Profile creation failed, but user account was created:", profileError);
            // Store profile data locally for later sync
            const profileData = {
            ...
            localStorage.setItem(`profile_${signUpData.user.id}`, JSON.stringify(profileData));
          }
  ...
  const getEnrolledCourses = useCallback(() => {
    return viewedCourses.filter(course => enrolledIds.includes(course.id));
  }, [viewedCourses, enrolledIds]);
    ...
    if (!isDemo && session && isSupabaseConfigured) {
      try {
        ...
        if (error) {
          ...
          localStorage.setItem(`enrolled_courses_${session.user.id}`, JSON.stringify(newEnrolledIds));
        } else {
          console.log("Enrollment updated successfully");
          // Clear localStorage fallback on success
          localStorage.removeItem(`enrolled_courses_${session.user.id}`);
        }
      } catch (err) {
             ...
             {isVerifying ? (
               <>
                 <div className="text-center mb-10">
                   <div className={`w-16 h-16 rounded-3xl flex items-center justify-center mx-auto mb-6 ${theme === 'dark' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-emerald-50 text-emerald-600'}`}><MailOpen className="w-8 h-8" /></div>
                   <h1 className={`text-3xl font-black mb-2 tracking-tight ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Account Created!</h1>
                   <p className="text-slate-400 font-bold text-[10px] uppercase tracking-widest">Almost Ready</p>
                </div>
                <div className={`p-6 rounded-xl text-center ${theme === 'dark' ? 'bg-slate-800' : 'bg-slate-50'}`}>
                   <p className={`text-sm font-medium mb-4 ${theme === 'dark' ? 'text-slate-300' : 'text-slate-600'}`}>
                     Your account has been created successfully!
                   </p>
                   <p className={`text-xs text-slate-400 mb-6`}>
                     If email verification is enabled, check your inbox. Otherwise, you can continue immediately.
                   </p>
                   <div className="space-y-3">
                     <button 
                       onClick={() => { setIsVerifying(false); setAuthMode('SIGNIN'); setSignupStep(1); setError(null); }}
                       className={`w-full font-black py-3 rounded-xl text-xs uppercase tracking-widest transition-all bg-indigo-600 text-white hover:bg-indigo-700`}
                     >
                       Continue to Login
                     </button>
                     <button 
                       onClick={() => { setIsVerifying(false); setAuthMode('SIGNIN'); setSignupStep(1); setError(null); }}
                       className={`w-full font-black py-3 rounded-xl text-xs uppercase tracking-widest transition-all border ${theme === 'dark' ? 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700' : 'bg-white border-slate-200 text-slate-500 hover:bg-slate-50'}`}
                     >
                       Back to Login
                     </button>
                   </div>
            ...
            <div className="space-y-10 animate-in fade-in pb-20">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 space-y-8">
                  <section className={`p-8 rounded-[2rem] border shadow-sm transition-colors ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
                    <h3 className={`text-base font-black mb-6 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Enrolled Courses</h3>
                    {getEnrolledCourses().length > 0 ? (
                      <div className="space-y-3">
                        {getEnrolledCourses().slice(0, 4).map(c => (
                          <div key={c.id} className={`flex items-center gap-3 p-3 rounded-xl transition-all border border-transparent cursor-pointer ${theme === 'dark' ? 'bg-slate-800 hover:bg-slate-700 hover:border-slate-600' : 'bg-slate-50 hover:bg-white hover:shadow-sm hover:border-slate-100'}`}>
                            ...
                            <div className="flex-1 min-w-0">
                               ...
                               <span className={`text-[7px] px-1 rounded font-black ${theme === 'dark' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-emerald-100 text-emerald-700'}`}>ACTIVE</span>
                            </div>
                          </div>
                        ))}
                      </div>
            ...
            <div className="space-y-10 animate-in fade-in pb-20">
              ...
              {courseViewMode === 'VAULT' ? (
                <>
                  {getEnrolledCourses().length > 0 && (
                    <>
                      <div className="bg-emerald-600 rounded-[2rem] p-8 text-white flex flex-col md:flex-row items-center gap-8 shadow-xl shadow-emerald-100 overflow-hidden relative animate-in fade-in duration-500">
                        <div className="relative z-10">
                          <h3 className="text-xl font-black mb-2">Enrolled Courses</h3>
                          <p className="text-emerald-100 text-xs font-medium leading-relaxed max-w-xl">
                            You have {getEnrolledCourses().length} enrolled course{getEnrolledCourses().length !== 1 ? 's' : ''}. These courses are actively tracked in your learning progress.
                          </p>
                        </div>
                        <CheckCircle2 className="absolute -right-10 -bottom-10 w-48 h-48 text-white/10" />
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in slide-in-from-bottom-4">
                        {getEnrolledCourses().map(course => (
                          <CourseCard key={course.id} course={course} theme={theme} isViewed={true} isEnrolled={true} onView={handleCourseView} onToggleEnroll={toggleEnrollment} />
                        ))}
                      </div>
                      <div className={`border-t ${theme === 'dark' ? 'border-slate-800' : 'border-slate-200'} my-12`}></div>
                    </>
                  )}
                  ...
                  <div className="bg-indigo-600 rounded-[2rem] p-8 text-white flex flex-col md:flex-row items-center gap-8 shadow-xl shadow-indigo-100 overflow-hidden relative animate-in fade-in duration-500">
                    <div className="relative z-10">
                      <h3 className="text-xl font-black mb-2">All Course History</h3>
                      <p className="text-indigo-100 text-xs font-medium leading-relaxed max-w-xl">
                        Complete collection of courses you've explored through assessments and discovery. Use the "Mark as Enrolled" button to track active learning.
                      </p>
                    </div>
                  ...
                  {viewedCourses.length > 0 ? (
                    ...
                    <div className={`rounded-[2rem] p-20 text-center border shadow-sm transition-colors ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
                      ...
                      <h4 className={`text-xl font-black mb-2 ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>No Courses Yet</h4>
                      <p className="text-slate-500 text-sm max-w-xs mx-auto font-medium mb-6">Take an assessment to get personalized course recommendations.</p>
